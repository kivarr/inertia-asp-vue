<template>
  <div class="h-full bg-gray-200 leading-none text-gray-700 antialiased text-left font-inter">
    <div id="dropdown"></div>
      ...
      <div class="md:h-screen md:flex md:flex-col" @click="hideDropdownMenus">
        <div class="md:flex md:flex-shrink-0">
          ...
          <div class="bg-white border-b w-full p-4 md:py-0 md:px-12 text-sm md:text-md flex justify-between items-center">
            <div class="mt-1 mr-4">{{ $page.props.share.user.account.name }}</div>
            <dropdown class="mt-1" placement="bottom-end">
              <div class="flex items-center cursor-pointer select-none group">
                <div class="text-gray-700 group-hover:text-indigo-600 focus:text-indigo-600 mr-1 whitespace-no-wrap">
                  <span>{{ $page.props.share.user.first_name }}&nbsp;</span>
                  <span class="hidden md:inline">{{ $page.props.share.user.last_name }}</span>
                </div>
              ...
              <template v-slot:dropdown>
                <div class="mt-2 py-2 shadow-xl bg-white rounded text-sm">
                  <inertia-link class="block px-6 py-2 hover:bg-indigo-500 hover:text-white text-center" href="#">My Profile</inertia-link>
                  <inertia-link class="block px-6 py-2 hover:bg-indigo-500 hover:text-white text-center" href="#">Manage Users</inertia-link>
                  <inertia-link class="block px-6 py-2 hover:bg-indigo-500 hover:text-white text-center" href="#" as="button" method="post">Logout</inertia-link>
                </div>
              </template>
            </dropdown>
          </div>
